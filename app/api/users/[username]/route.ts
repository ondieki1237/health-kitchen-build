...
export async function GET(request: NextRequest, { params }: { params: { username: string } }) {
  try {
    ...
    // Get user's recipes
    const Recipe = (await import("@/server/models/Recipe")).default
    const userRecipes = await Recipe.find({ createdBy: user._id }).limit(20).sort({ createdAt: -1 })

    // Get user's menus
    const Menu = (await import("@/server/models/Menu")).default
    const userMenus = await Menu.find({ createdBy: user._id, isPublic: true })
      .limit(20)
      .sort({ createdAt: -1 })

    // Get user's cooking attempts
    const CookingAttempt = (await import("@/server/models/CookingAttempt")).default
